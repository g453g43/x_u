...
Instance.new("UICorner", OO_0).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", OO_0).Color = O_O0.Line

local AuthTitle = Instance.new("TextLabel", OO_0); AuthTitle.Size = UDim2.new(1, 0, 0, 40); AuthTitle.BackgroundTransparency = 1
...
OO00.TextColor3 = O_O0.Text; OO00.TextTransparency = 1 -- Hide the real text!
OO00.Font = Enum.Font.Gotham; OO00.TextSize = 13; OO00.TextXAlignment = Enum.TextXAlignment.Left
OO00.ClearTextOnFocus = false; OO00.ZIndex = 2

OO00:GetPropertyChangedSignal("Text"):Connect(function()
    FakeLabel.Text = string.rep("*", #OO00.Text)
    if #OO00.Text == 0 then
        FakeLabel.Text = ""
        OO00.PlaceholderText = "Enter Key..."
    else
        OO00.PlaceholderText = ""
    end
end)

local AuthBtn = Instance.new("TextButton", OO_0)
...
Instance.new("UIStroke", Main).Color = O_O0.Line
...
make_draggable(Main); make_draggable(OO_0)

local IsAuthenticated = false
AuthBtn.MouseButton1Click:Connect(function()
    if O0_O(OO00.Text) then
        IsAuthenticated = true; OO_0:Destroy(); Main.Visible = true; print("x_u: Access Granted.")
    else
        OO00.Text = ""; FakeLabel.Text = ""; OO00.PlaceholderText = "Invalid Key!"; task.wait(1.5); OO00.PlaceholderText = "Enter Key..."
    end
end)
...
Instance.new("Frame", Main).Size = UDim2.new(0, 1, 1, -50); Main.Frame.Position = UDim2.new(0, 140, 0, 50); Main.Frame.BackgroundColor3 = O_O0.Line; Main.Frame.BorderSizePixel = 0
...
local Tabs = {}
local function SelectTab(topName, sideName) pcall(function() for _, t in pairs(Tabs) do t.SideCont.Visible = false; t.TopBtn.TextColor3 = O_O0.TextDark; if t.TopBtn:FindFirstChild("Line") then t.TopBtn.Line.Visible = false end; for _, s in pairs(t.Sides) do s.Page.Visible = false; s.Btn.TextColor3 = O_O0.TextDark; if s.Btn:FindFirstChild("Line") then s.Btn.Line.Visible = false end end end; local top = Tabs[topName]; top.TopBtn.TextColor3 = O_O0.Text; if top.TopBtn:FindFirstChild("Line") then top.TopBtn.Line.Visible = true end; top.SideCont.Visible = true; local side = top.Sides[sideName]; side.Btn.TextColor3 = O_O0.Text; if side.Btn:FindFirstChild("Line") then side.Btn.Line.Visible = true end; side.Page.Visible = true end) end
local function CreateTopTab(name) local Btn = Instance.new("TextButton", TopTabCont); Btn.Size = UDim2.new(0, 70, 0, 50); Btn.BackgroundTransparency = 1; Btn.Text = name; Btn.Font = Enum.Font.GothamMedium; Btn.TextSize = 13; Btn.TextColor3 = O_O0.TextDark; local Line = Instance.new("Frame", Btn); Line.Name = "Line"; Line.Size = UDim2.new(1, 0, 0, 2); Line.Position = UDim2.new(0, 0, 1, -2); Line.BackgroundColor3 = O_O0.Accent; Line.BorderSizePixel = 0; Line.Visible = false; local SCont = Instance.new("Frame", SideTabCont); SCont.Size = UDim2.new(1, 0, 1, 0); SCont.BackgroundTransparency = 1; SCont.Visible = false; Instance.new("UIListLayout", SCont).Padding = UDim.new(0, 2); SCont.UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; Tabs[name] = { TopBtn = Btn, Sides = {}, SideCont = SCont }; Btn.MouseButton1Click:Connect(function() local f = next(Tabs[name].Sides) if f then SelectTab(name, f) end end); return name end
local function CreateSideTab(topName, name) local Btn = Instance.new("TextButton", Tabs[topName].SideCont); Btn.Size = UDim2.new(1, -15, 0, 35); Btn.BackgroundTransparency = 1; Btn.Text = "  " .. name; Btn.Font = Enum.Font.Gotham; Btn.TextSize = 13; Btn.TextColor3 = O_O0.TextDark; Btn.TextXAlignment = Enum.TextXAlignment.Left; local Line = Instance.new("Frame", Btn); Line.Name = "Line"; Line.Size = UDim2.new(0, 3, 1, -12); Line.Position = UDim2.new(0, 0, 0, 6); Line.BackgroundColor3 = O_O0.Accent; Line.BorderSizePixel = 0; Line.Visible = false; local Page = Instance.new("Frame", ContentCont); Page.Size = UDim2.new(1, -40, 1, -40); Page.Position = UDim2.new(0, 20, 0, 20); Page.BackgroundTransparency = 1; Page.Visible = false; Instance.new("UIListLayout", Page).Padding = UDim.new(0, 12); Tabs[topName].Sides[name] = { Btn = Btn, Page = Page }; Btn.MouseButton1Click:Connect(function() SelectTab(topName, name) end); return Page end
local function AddToggle(parent, text, default, callback) local Frame = Instance.new("TextButton", parent); Frame.Size = UDim2.new(1, 0, 0, 25); Frame.BackgroundTransparency = 1; Frame.Text = ""; local L = Instance.new("TextLabel", Frame); L.Text = text; L.Font = Enum.Font.Gotham; L.TextColor3 = O_O0.Text; L.TextSize = 13; L.Size = UDim2.new(1, -45, 1, 0); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Left; local BG = Instance.new("Frame", Frame); BG.Size = UDim2.new(0, 32, 0, 16); BG.Position = UDim2.new(1, -32, 0.5, -8); BG.BackgroundColor3 = default and O_O0.Accent or O_O0.Btn; Instance.new("UICorner", BG).CornerRadius = UDim.new(1,0); local Knob = Instance.new("Frame", BG); Knob.Size = UDim2.new(0, 12, 0, 12); Knob.Position = default and UDim2.new(1,-14,0.5,-6) or UDim2.new(0,2,0.5,-6); Knob.BackgroundColor3 = Color3.fromRGB(245,245,255); Instance.new("UICorner", Knob).CornerRadius = UDim.new(1,0); local s = default; Frame.MouseButton1Click:Connect(function() pcall(function() s = not s; TS:Create(BG, TweenInfo.new(0.2), {BackgroundColor3 = s and O_O0.Accent or O_O0.Btn}):Play(); TS:Create(Knob, TweenInfo.new(0.2), {Position = s and UDim2.new(1,-14,0.5,-6) or UDim2.new(0,2,0.5,-6)}):Play(); callback(s) end) end) end
local function AddSlider(parent, text, default, min, max, callback) local F = Instance.new("Frame", parent); F.Size = UDim2.new(1, 0, 0, 30); F.BackgroundTransparency = 1; local L = Instance.new("TextLabel", F); L.Text = text; L.Font = Enum.Font.Gotham; L.TextColor3 = O_O0.Text; L.TextSize = 12; L.Size = UDim2.new(1, 0, 0, 12); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Left; local V = Instance.new("TextLabel", F); V.Text = tostring(default); V.Font = Enum.Font.Gotham; V.TextColor3 = O_O0.TextDark; V.TextSize = 12; V.Size = UDim2.new(1, 0, 0, 12); V.BackgroundTransparency = 1; V.TextXAlignment = Enum.TextXAlignment.Right; local Track = Instance.new("TextButton", F); Track.Size = UDim2.new(1, 0, 0, 2); Track.Position = UDim2.new(0, 0, 1, -6); Track.BackgroundColor3 = O_O0.Btn; Track.Text = ""; local Fill = Instance.new("Frame", Track); Fill.Size = UDim2.new((default-min)/(max-min), 0, 1, 0); Fill.BackgroundColor3 = O_O0.Accent; Fill.BorderSizePixel = 0; local d = false; Track.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then d = true end end); UIS.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then d = false end end); UIS.InputChanged:Connect(function(i) if d and i.UserInputType == Enum.UserInputType.MouseMovement then pcall(function() local p = math.clamp((i.Position.X-Track.AbsolutePosition.X)/Track.AbsoluteSize.X,0,1); Fill.Size = UDim2.new(p,0,1,0); local val = math.floor(min+(max-min)*p); V.Text = tostring(val); callback(val) end) end end) end
local ESPObjects = {}
...
local function SetupESP(p) if p == LP then return end ClearESP(p) pcall(function() local box = Drawing.new("Square"); box.Visible = false; box.Thickness = 1; box.Color = O_O0.Accent; local text = Drawing.new("Text"); text.Visible = false; text.Size = 13; text.Center = true; text.Outline = true; text.Color = O_O0.Text; ESPObjects[p] = { Box = box, Text = text } end) end
...
local function get_near() local near, d = nil, math.huge pcall(function() for _, p in pairs(Players:GetPlayers()) do if p ~= LP and p.Character and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 and p.Character:FindFirstChild(O_0O.TargetPart) then local pos, vis = Camera:WorldToViewportPoint(p.Character[O_0O.TargetPart].Position) if vis then local m_d = (Vector2.new(pos.X, pos.Y) - Vector2.new(Mouse.X, Mouse.Y)).Magnitude if m_d < d and m_d <= O_0O.FOVRadius then d = m_d; near = p end end end end end) return near end
local hooked = false
local function ApplyHook() if hooked then return end pcall(function() local old; old = hookmetamethod(LP:GetMouse(), "__index", newcclosure(function(self, index) if not checkcaller() and O_0O.SilentAim and target and target.Character then if index == "Hit" then return target.Character[O_0O.TargetPart].CFrame + (target.Character[O_0O.TargetPart].Velocity * O_0O.Prediction) elseif index == "Target" then return target.Character[O_0O.TargetPart] end end return old(self, index) end)); hooked = true end) end
local T1 = CreateTopTab("Aim"); local T2 = CreateTopTab("Visuals"); local T3 = CreateTopTab("Utility")
local S1 = CreateSideTab(T1, "Combat"); AddToggle(S1, "Silent Aim", false, function(v) O_0O.SilentAim = v; if v then ApplyHook() end end); AddToggle(S1, "Hard Aimlock", false, function(v) O_0O.HardLock = v end); AddSlider(S1, "Prediction", 165, 0, 400, function(v) O_0O.Prediction = v/1000 end)
local S2 = CreateSideTab(T2, "ESP"); AddToggle(S2, "Enable ESP", false, function(v) O_0O.ESPEnabled = v end); AddToggle(S2, "Show Boxes", false, function(v) O_0O.ESPBoxes = v end); AddToggle(S2, "Show Names", false, function(v) O_0O.ESPNames = v end)
...
local function UpdateList() for _, c in pairs(SScroll:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end for _, p in pairs(Players:GetPlayers()) do if p ~= LP then local B = Instance.new("TextButton", SScroll); B.Size = UDim2.new(1,-10,0,30); B.BackgroundColor3 = O_O0.Btn; B.Text = p.Name; B.TextColor3 = O_O0.Text; B.Font = Enum.Font.Gotham; B.TextSize = 12; Instance.new("UICorner", B); B.MouseButton1Click:Connect(function() pcall(function() if cur_spec == p then cur_spec = nil; Camera.CameraSubject = LP.Character.Humanoid; B.TextColor3 = O_O0.Text else cur_spec = p; Camera.CameraSubject = p.Character.Humanoid; B.TextColor3 = O_O0.Accent; for _, x in pairs(SScroll:GetChildren()) do if x:IsA("TextButton") and x ~= B then x.TextColor3 = O_O0.Text end end end end) end) end end SScroll.CanvasSize = UDim2.new(0,0,0,SScroll.UIListLayout.AbsoluteContentSize.Y) end
UpdateList(); Players.PlayerAdded:Connect(UpdateList); Players.PlayerRemoving:Connect(UpdateList)
local S4 = CreateSideTab(T3, "Misc"); AddToggle(S4, "Rapid Fire", false, function(v) O_0O.RapidFire = v end); AddToggle(S4, "Void Hide", false, function(v) O_0O.VoidHide = v end)
local S6 = CreateSideTab(T3, "Movement"); AddToggle(S6, "Fly Toggle", false, function(v) O_0O.FlyEnabled = v end); AddToggle(S6, "Speed Toggle", false, function(v) O_0O.SpeedEnabled = v end)
local T4 = CreateTopTab("O_0O"); local S5 = CreateSideTab(T4, "Main"); local Unload = Instance.new("TextButton", S5); Unload.Size = UDim2.new(1, 0, 0, 30); Unload.BackgroundColor3 = Color3.fromRGB(50, 20, 20); Unload.Text = "Unload"; Unload.TextColor3 = Color3.fromRGB(240, 100, 100); Unload.Font = Enum.Font.GothamBold; Unload.Parent = S5; Unload.MouseButton1Click:Connect(function() UI:Destroy() end)
RS.RenderStepped:Connect(function() if not UI.Parent or not IsAuthenticated then return end target = get_near(); if O_0O.HardLock and target and target.Character then Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, target.Character[O_0O.TargetPart].Position), O_0O.Smoothing) end; for p, h in pairs(ESPObjects) do local char = p.Character; local hum = char and char:FindFirstChild("Humanoid") if O_0O.ESPEnabled and char and hum and hum.Health > 0 and char:FindFirstChild("HumanoidRootPart") then local pos, vis = Camera:WorldToViewportPoint(char.HumanoidRootPart.Position) if vis then local s = 1000/pos.Z; h.Box.Visible = O_0O.ESPBoxes; h.Box.Size = Vector2.new(4,6)*s; h.Box.Position = Vector2.new(pos.X-h.Box.Size.X/2, pos.Y-h.Box.Size.Y/2); h.Text.Visible = O_0O.ESPNames; h.Text.Text = p.Name; h.Text.Position = Vector2.new(pos.X, pos.Y-h.Box.Size.Y/2-15) else h.Box.Visible = false; h.Text.Visible = false end else h.Box.Visible = false; h.Text.Visible = false end end; if O_0O.VoidHide and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then local hrp = LP.Character.HumanoidRootPart; local t = tick()*15; hrp.CFrame = CFrame.new(hrp.Position.X+math.sin(t)*15, -100000, hrp.Position.Z+math.cos(t)*15); hrp.Velocity = Vector3.new(0,0,0) end end)
task.spawn(function() while true do RS.Heartbeat:Wait() if O_0O.RapidFire and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and IsAuthenticated then local t = LP.Character and LP.Character:FindFirstChildOfClass("Tool") if t then for i=1,5 do task.spawn(function() pcall(function() t:Activate() end) end) end end end end)
UIS.InputBegan:Connect(function(i, g) if not g and IsAuthenticated then if i.KeyCode == O_0O.ToggleKey then Main.Visible = not Main.Visible end end end)
SelectTab(T1, "Combat")
