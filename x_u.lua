-- x_u private (Premium Configuration & Mechanics Build)
...
local Config = {
    ...
    SilentAim = false, SilentAimKey = "E", HardLock = false, HardLockKey = "Q", TargetPart = "Head",
    Triggerbot = false, TriggerbotKey = "T", TriggerbotMode = "Hold",
    ESPEnabled = false, ESPBoxes = false, ESPNames = false,
    RapidFire = false, VoidHide = false,
    FlyEnabled = false, FlyKey = "F", FlySpeed = 150,
    SpeedEnabled = false, SpeedKey = "Z", SpeedValue = 150
}

-- Load/Save System
local function SaveConfig()
    pcall(function()
        if not isfolder("xu_configs") then makefolder("xu_configs") end
        writefile("xu_configs/" .. Config.ConfigName .. ".json", Http:JSONEncode(Config))
        print("x_u: Saved config " .. Config.ConfigName)
    end)
end
local function LoadConfig()
    pcall(function()
        if isfile("xu_configs/" .. Config.ConfigName .. ".json") then
            local data = readfile("xu_configs/" .. Config.ConfigName .. ".json")
            local parsed = Http:JSONDecode(data)
            for k,v in pairs(parsed) do Config[k] = v end
            print("x_u: Loaded config " .. Config.ConfigName)
        else
            print("x_u: Config not found!")
        end
    end)
end
...
AuthMain.Size = UDim2.new(0, 300, 0, 180); AuthMain.Position = UDim2.new(0.5, -150, 0.5, -90); AuthMain.BackgroundColor3 = Theme.BG; AuthMain.BorderSizePixel = 0; AuthMain.Active = true
Instance.new("UICorner", AuthMain).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", AuthMain).Color = Theme.Line
local AuthTitle = Instance.new("TextLabel", AuthMain); AuthTitle.Size = UDim2.new(1, 0, 0, 40); AuthTitle.BackgroundTransparency = 1; AuthTitle.Text = "PRIVATE ACCESS"; AuthTitle.Font = Enum.Font.GothamBold; AuthTitle.TextSize = 16; AuthTitle.TextColor3 = Theme.Accent
local f1 = Instance.new("Frame", AuthMain); f1.Size = UDim2.new(1, 0, 0, 1); f1.Position = UDim2.new(0, 0, 0, 40); f1.BackgroundColor3 = Theme.Line; f1.BorderSizePixel = 0

local InputContainer = Instance.new("Frame", AuthMain)
InputContainer.Size = UDim2.new(1, -40, 0, 35); InputContainer.Position = UDim2.new(0, 20, 0, 65); InputContainer.BackgroundColor3 = Theme.Btn
...
local AuthBtn = Instance.new("TextButton", AuthMain); AuthBtn.Size = UDim2.new(1, -40, 0, 35); AuthBtn.Position = UDim2.new(0, 20, 0, 115); AuthBtn.BackgroundColor3 = Theme.Line; AuthBtn.Text = "Login"; AuthBtn.TextColor3 = Theme.Text; AuthBtn.Font = Enum.Font.GothamMedium; AuthBtn.TextSize = 14
Instance.new("UICorner", AuthBtn).CornerRadius = UDim.new(0, 4)

-- // MAIN CHEAT UI
local Main = Instance.new("Frame", UI); Main.Size = UDim2.new(0, 600, 0, 450); Main.Position = UDim2.new(0.5, -300, 0.5, -225); Main.BackgroundColor3 = Theme.BG; Main.BorderSizePixel = 0; Main.Active = true; Main.Visible = false
...
AuthBtn.MouseButton1Click:Connect(function()
    ...
    if user then
        IsAuthenticated = true; InputContainer.Visible = false
        for i = 1, 4 do AuthBtn.Text = "Welcome " .. user .. string.rep(".", i % 4); task.wait(0.35) end
        AuthMain:Destroy(); Main.Visible = true; LoadConfig() -- Auto-load on inject completion
    else
...
local function AddKeybind(parent, text, defaultVal, callback) local F = Instance.new("Frame", parent); F.Size = UDim2.new(1, 0, 0, 25); F.BackgroundTransparency = 1; local L = Instance.new("TextLabel", F); L.Text = text; L.Font = Enum.Font.Gotham; L.TextColor3 = Theme.Text; L.TextSize = 12; L.Size = UDim2.new(1, -70, 1, 0); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Left; local B = Instance.new("TextButton", F); B.Size = UDim2.new(0, 70, 0, 20); B.Position = UDim2.new(1, -70, 0.5, -10); B.BackgroundColor3 = Theme.Btn; B.Text = defaultVal; B.TextColor3 = Theme.Text; B.Font = Enum.Font.Gotham; B.TextSize = 11; Instance.new("UICorner", B).CornerRadius = UDim.new(0, 4); local W = false; B.MouseButton1Click:Connect(function() W = true; B.Text = "..." end); UIS.InputBegan:Connect(function(i) if W and i.UserInputType == Enum.UserInputType.Keyboard then W = false; B.Text = i.KeyCode.Name; callback(i.KeyCode.Name) end end) end
local function AddStringBox(parent, text, defaultVal, callback) local F = Instance.new("Frame", parent); F.Size = UDim2.new(1, 0, 0, 30); F.BackgroundTransparency = 1; local L = Instance.new("TextLabel", F); L.Text = text; L.Font = Enum.Font.Gotham; L.TextColor3 = Theme.Text; L.TextSize = 12; L.Size = UDim2.new(1, -120, 1, 0); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Left; local B = Instance.new("TextBox", F); B.Size = UDim2.new(0, 120, 0, 22); B.Position = UDim2.new(1, -120, 0.5, -11); B.BackgroundColor3 = Theme.Btn; B.Text = defaultVal; B.TextColor3 = Theme.Text; B.Font = Enum.Font.Gotham; B.TextSize = 11; Instance.new("UICorner", B).CornerRadius = UDim.new(0, 4); B.FocusLost:Connect(function() callback(B.Text) end) end
local function AddDropdown(parent, text, options, defaultVal, callback) local F = Instance.new("Frame", parent); F.Size = UDim2.new(1, 0, 0, 25); F.BackgroundTransparency = 1; local L = Instance.new("TextLabel", F); L.Text = text; L.Font = Enum.Font.Gotham; L.TextColor3 = Theme.Text; L.TextSize = 12; L.Size = UDim2.new(1, -90, 1, 0); L.BackgroundTransparency = 1; L.TextXAlignment = Enum.TextXAlignment.Left; local B = Instance.new("TextButton", F); B.Size = UDim2.new(0, 90, 0, 20); B.Position = UDim2.new(1, -90, 0.5, -10); B.BackgroundColor3 = Theme.Btn; B.Text = defaultVal; B.TextColor3 = Theme.Text; B.Font = Enum.Font.Gotham; B.TextSize = 11; Instance.new("UICorner", B).CornerRadius = UDim.new(0, 4); local idx = 1; for i,v in pairs(options) do if v == defaultVal then idx = i break end end; B.MouseButton1Click:Connect(function() idx = idx + 1; if idx > #options then idx = 1 end; B.Text = options[idx]; callback(options[idx]) end) end
local function AddButton(parent, text, callback) local B = Instance.new("TextButton", parent); B.Size = UDim2.new(1, 0, 0, 30); B.BackgroundColor3 = Theme.Line; B.Text = text; B.TextColor3 = Theme.Text; B.Font = Enum.Font.GothamBold; B.TextSize = 13; Instance.new("UICorner", B).CornerRadius = UDim.new(0, 4); B.MouseButton1Click:Connect(callback) end
...
local function get_near() local near, d = nil, math.huge pcall(function() for _, p in pairs(Players:GetPlayers()) do if p ~= LP and p.Character and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 and p.Character:FindFirstChild(Config.TargetPart) then local pos, vis = Camera:WorldToViewportPoint(p.Character[Config.TargetPart].Position) if vis then local m_d = (Vector2.new(pos.X, pos.Y) - Vector2.new(Mouse.X, Mouse.Y)).Magnitude if m_d < d and m_d <= 100 then d = m_d; near = p end end end end end) return near end

-- Combat Aim Hook
local hooked = false
local function ApplyHook() if hooked then return end pcall(function() local old; old = hookmetamethod(LP:GetMouse(), "__index", newcclosure(function(self, index) if not checkcaller() and Config.SilentAim and target and target.Character then if index == "Hit" or index == "Target" then return target.Character[Config.TargetPart].CFrame end end return old(self, index) end)); hooked = true end) end

-- UI Layouts
local T1 = CreateTopTab("Aimbot"); local T2 = CreateTopTab("Visuals"); local T3 = CreateTopTab("Utility")

local S1 = CreateSideTab(T1, "Combat")
AddToggle(S1, "Silent Aim", Config.SilentAim, function(v) Config.SilentAim = v; if v then ApplyHook() end end)
AddKeybind(S1, "Silent Aim Key", Config.SilentAimKey, function(v) Config.SilentAimKey = v end)
AddToggle(S1, "Hard Aimlock", Config.HardLock, function(v) Config.HardLock = v end)
AddKeybind(S1, "Hardlock Key", Config.HardLockKey, function(v) Config.HardLockKey = v end)

local S1_B = CreateSideTab(T1, "Triggerbot")
AddToggle(S1_B, "Enable Triggerbot", Config.Triggerbot, function(v) Config.Triggerbot = v end)
AddDropdown(S1_B, "Trigger Mode", {"Hold", "Toggle", "Always On"}, Config.TriggerbotMode, function(v) Config.TriggerbotMode = v end)
AddKeybind(S1_B, "Triggerbot Bind", Config.TriggerbotKey, function(v) Config.TriggerbotKey = v end)

local S2 = CreateSideTab(T2, "ESP")
AddToggle(S2, "Enable ESP", Config.ESPEnabled, function(v) Config.ESPEnabled = v end)
AddToggle(S2, "Show Boxes", Config.ESPBoxes, function(v) Config.ESPBoxes = v end)
AddToggle(S2, "Show Names", Config.ESPNames, function(v) Config.ESPNames = v end)
...
local S4 = CreateSideTab(T3, "Misc")
AddToggle(S4, "Rapid Fire (Insane)", Config.RapidFire, function(v) Config.RapidFire = v end)
AddToggle(S4, "Void Hide (-100K Y)", Config.VoidHide, function(v) Config.VoidHide = v end)

local S6 = CreateSideTab(T3, "Movement")
AddToggle(S6, "Fly Toggle", Config.FlyEnabled, function(v) Config.FlyEnabled = v end)
AddKeybind(S6, "Fly Bind", Config.FlyKey, function(v) Config.FlyKey = v end)
AddSlider(S6, "Fly Speed Maximum", Config.FlySpeed, 50, 500, function(v) Config.FlySpeed = v end)
AddToggle(S6, "Speed Toggle", Config.SpeedEnabled, function(v) Config.SpeedEnabled = v end)
AddKeybind(S6, "Speed Bind", Config.SpeedKey, function(v) Config.SpeedKey = v end)
AddSlider(S6, "Speed Value Maximum", Config.SpeedValue, 50, 500, function(v) Config.SpeedValue = v end)

local T4 = CreateTopTab("Settings")
local S5 = CreateSideTab(T4, "Main")
AddKeybind(S5, "Menu Toggle Key", Config.MenuKey, function(v) Config.MenuKey = v end)
AddToggle(S5, "Stream Proof GUI", Config.StreamProof, function(v) Config.StreamProof = v; pcall(function() if v then UI.Parent = game:GetService("CoreGui") else UI.Parent = LP:WaitForChild("PlayerGui") end end) end)
AddButton(S5, "Force Unload Client", function() UI:Destroy() end)

local S7 = CreateSideTab(T4, "Configs")
AddStringBox(S7, "File Name", Config.ConfigName, function(v) Config.ConfigName = v end)
AddButton(S7, "Save Config", SaveConfig)
AddButton(S7, "Load Config", LoadConfig)

-- Game Loop Engine
local trigger_toggle_state = false
local aimlock_toggle_state = false
local silentaim_toggle_state = false

RS.RenderStepped:Connect(function() 
    if not UI.Parent or not IsAuthenticated then return end 
    target = get_near()
    
    -- Hard Aimlock (Tight Snap)
    if Config.HardLock and target and target.Character then 
        local t_pos = target.Character[Config.TargetPart].Position
        -- Increased smoothing internally inside CFrame.Lerp for a harder lock (0.9 vs 0.5)
        Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, t_pos), 0.95) 
    end
    
    -- ESP Draw
    for p, h in pairs(ESPObjects) do 
        local char = p.Character; local hum = char and char:FindFirstChild("Humanoid") 
        if Config.ESPEnabled and char and hum and hum.Health > 0 and char:FindFirstChild("HumanoidRootPart") then 
            local pos, vis = Camera:WorldToViewportPoint(char.HumanoidRootPart.Position) 
            if vis then 
                local s = 1000/pos.Z; h.Box.Visible = Config.ESPBoxes; h.Box.Size = Vector2.new(4,6)*s; h.Box.Position = Vector2.new(pos.X-h.Box.Size.X/2, pos.Y-h.Box.Size.Y/2); h.Text.Visible = Config.ESPNames; h.Text.Text = p.Name; h.Text.Position = Vector2.new(pos.X, pos.Y-h.Box.Size.Y/2-15) 
            else h.Box.Visible = false; h.Text.Visible = false end 
        else h.Box.Visible = false; h.Text.Visible = false end 
    end
    
    -- Void Hide
    if Config.VoidHide and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then 
        local hrp = LP.Character.HumanoidRootPart; local t = tick()*15; hrp.CFrame = CFrame.new(hrp.Position.X+math.sin(t)*15, -100000, hrp.Position.Z+math.cos(t)*15); hrp.Velocity = Vector3.new(0,0,0) 
    end 
end)
...
task.spawn(function() 
    while true do 
        ...
        -- Fly
        if Config.FlyEnabled and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            hrp.Velocity = vec * Config.FlySpeed
            hrp.CFrame = CFrame.new(hrp.Position, hrp.Position + cam.CFrame.LookVector) -- Halt rotational drift
        end
        
        -- Speed (Velocity Overdrive)
        if Config.SpeedEnabled and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") and not Config.FlyEnabled then
            local hrp = LP.Character.HumanoidRootPart
        ...
        -- Rapid Fire (Accelerated Iteration)
        if Config.RapidFire and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then 
            local t = LP.Character and LP.Character:FindFirstChildOfClass("Tool") 
            if t then for i=1,20 do task.spawn(function() pcall(function() t:Activate() end) end) end end 
        end 

        -- Triggerbot Logic Raycasting
        if Config.Triggerbot then
            local active = false
            if Config.TriggerbotMode == "Always On" then active = true
            elseif Config.TriggerbotMode == "Hold" then active = UIS:IsKeyDown(Enum.KeyCode[Config.TriggerbotKey] or Enum.KeyCode.T)
            elseif Config.TriggerbotMode == "Toggle" then active = trigger_toggle_state end

            if active and Mouse.Target and Mouse.Target.Parent:FindFirstChild("Humanoid") and Mouse.Target.Parent.Humanoid.Health > 0 then
...
UIS.InputBegan:Connect(function(i, g) 
    if not g and IsAuthenticated then 
        if i.KeyCode.Name == Config.MenuKey then Main.Visible = not Main.Visible end 
        if i.KeyCode.Name == Config.SpeedKey then Config.SpeedEnabled = not Config.SpeedEnabled end
        if i.KeyCode.Name == Config.FlyKey then Config.FlyEnabled = not Config.FlyEnabled end
        if i.KeyCode.Name == Config.SilentAimKey then Config.SilentAim = not Config.SilentAim; if Config.SilentAim then ApplyHook() end end
        if i.KeyCode.Name == Config.HardLockKey then Config.HardLock = not Config.HardLock end
        if i.KeyCode.Name == Config.TriggerbotKey then trigger_toggle_state = not trigger_toggle_state end
    end 
end)

SelectTab(T1, "Combat")
print("--- x_u private Engine Ready ---")
end)
